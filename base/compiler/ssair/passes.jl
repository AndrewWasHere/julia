...
import .EscapeAnalysis: EscapeInfo, Indexable, getaliases
...
function sroa_mutables!(ir::IRCode, nargs::Int)
    ...
    eliminated = BitSet()
    revisit = Tuple{#=related=#Vector{SSAValue}, #=liveness=#BitSet}[]
    all_preserved = true
    ...
    while !isempty(wset)
        ...
        finfos = (einfo.AliasInfo::Indexable).infos
        ...
        for livepc in Liveness
            ...
            elseif isexpr(livestmt, :foreigncall) # preserve use (otherwise not is_load_forwarded)
        ...
        for fidx in 1:nfields
            ...
            for use in fdu.uses
                ...
                elseif all_preserved && isa(use, PreserveUse)
                    ...
                    use = getuseidx(use)
                    push!(get!(()->Any[], newpreserves, use), compute_value_for_use(
                        ir, domtree, allblocks, fdu, phinodes, fidx, use))
    ...
    mark_dead_ssas!(ir, deadssas, revisit, eliminated)
    for ssa in deadssas
        ir[ssa][:inst] = nothing
    end
...
function is_load_forwardable(x::EscapeInfo)
    ...
    return isa(AliasInfo, Indexable) && !AliasInfo.array
end
...
function mark_dead_ssas!(ir::IRCode, deadssas::IdSet{SSAValue},
    revisit::Vector{Tuple{Vector{SSAValue},BitSet}}, eliminated::BitSet)
    wset = BitSet(1:length(revisit))
    while !isempty(wset)
        revisit_idx = pop!(wset)
        mark_dead_ssas!(ir, deadssas, revisit, eliminated, wset, revisit_idx)
    end
end

function mark_dead_ssas!(ir::IRCode, deadssas::IdSet{SSAValue},
    revisit::Vector{Tuple{Vector{SSAValue},BitSet}}, eliminated::BitSet, wset::BitSet, revisit_idx::Int)
        ...
        if isexpr(stmt, :new)
            ...
            for new_revisit_idx in wset
                if ssa in revisit[new_revisit_idx][1]
                    ...
                    if mark_dead_ssas!(ir, deadssas, revisit, eliminated, wset, new_revisit_idx)
                        push!(eliminable, ssa)
                        @goto next_live
                    else
                        return false
